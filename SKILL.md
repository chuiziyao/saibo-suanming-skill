---
name: 赛博算命
description: 赛博算命 - 中国传统命理八字排盘与运势预测工具（v2.5 零依赖，装完即用）。当用户需要根据生辰八字进行算命、测算、推演命运、查看流年运势、查询财运、官运、婚姻运、子女运、健康运时使用此技能。用户提供公历出生年月日时、性别、出生地后，本技能将排盘四柱八字、推算大运、逐年预测运势，并给出趋吉避凶建议。内置1900-2100年节气表，日柱基于验证基准日精算，无需安装任何外部依赖。
author: 锤无双
version: 2.5.0
---
...
> 🚀 **v2.5 零依赖！** 内置 1900-2100 年节气表，日柱基于验证基准日（1900-01-01=甲戌）精算，装完即用，不需安装任何外部包。
>
> **v2.5 更新**：身强身弱判定全面升级——新增克泄耗扣分、地支合冲刑衰减、五档阈值+从格前置判断。

## 概述

本技能基于中国传统命理学（四柱八字、子平命学），内置 1900-2100 年节气表，零依赖。用户输入公历出生年月日时、性别、出生地后，将获得：

**v2.5 更新**：
- 重写 `analyze_xiyongshen()` 身强/身弱判定算法（7步完整流程）
- 新增天干+藏干双层克泄耗扣分（官杀/财星/食伤），解决「只加不减」失真
- 新增地支合冲刑衰减修正（六冲×0.5、六合×0.6、三合×0.5、三会×0.4、相刑×0.5）
- 新增特殊格局前置判断（从强/从弱/假从），优先级高于常规打分
- 五档阈值升级：身强 ≥6.0 / 中和偏强 ≥4.5 / 标准中和 ≥3.5 / 中和偏弱 ≥2.0 / 身弱 <2.0

**v2.1 更新**：
- 内置 1900-2100 年节气表，去除 sxtwl 外部依赖，实现零依赖装完即用
...
### scripts/八字排盘.py
核心排盘计算引擎 **v2.2**（零依赖），包含：
- `calc_bazi()` - 四柱排盘（日柱基于验证基准日 1900-01-01=甲戌 精算）
- `calc_dayun()` - 大运推算（起运基于内置节气表天数÷3）
- `get_shishen()` - 十神关系
- `get_shensha()` - 神煞查询
- `get_geju()` - 格局判断
- `analyze_xiyongshen()` - **v2.5 重写**：7步完整流程，含克泄耗扣分+合冲刑衰减+五档阈值+从格判断
